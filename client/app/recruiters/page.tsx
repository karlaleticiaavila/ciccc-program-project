...
export default function RecruitersPage() {
  ...
  const fetchPublicMilestones = useCallback(
    async () => {
      try {
        setError("");
        ...
        if (!response.ok) {
          throw new Error(
            "Could not load public journeys."
          );
        }
  ...
  const candidates = useMemo<Candidate[]>(() => {
    ...
    milestones.forEach((milestone) => {
      ...
      if (existingCandidate) {
        existingCandidate.milestones.push(
          milestone
        );
  ...
  const selectedFeaturedEvidence =
    useMemo(() => {
  ...
  return (
    <main className="min-h-screen bg-[#123b40] text-[#f5efe3]">
      ...
      {/* HERO */}
      <section className="border-b border-[#f5efe3]/10 px-5 pb-16 pt-40 md:px-10 md:pb-24 md:pt-36">
          ...
          <div className="grid gap-14 xl:grid-cols-[1.25fr_0.75fr] xl:items-end">
            ...
            <div className="max-w-lg xl:justify-self-end">
              <p className="text-lg leading-8 text-[#f5efe3]/65">
                Discover the people behind the
                experience. Explore public
                journeys, meaningful milestones
                and the stories that rarely fit
                on a résumé.
              </p>
