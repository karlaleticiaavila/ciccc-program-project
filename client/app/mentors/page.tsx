...
export default function MentorsPage() {
  ...
  useEffect(() => {
    const loadMilestones = async () => {
      try {
        ...
        setError(
          "We could not load the mentor space."
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleFeedbackSubmit = async (
    ...
    const message =
      messages[milestoneId]?.trim();

    if (!message) return;

    if (!session?.accessToken) {
      setError(
        "Please sign in before leaving feedback."
      );
      return;
    }
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.3fr_0.7fr] lg:items-end">
            ...
            <p className="max-w-md text-sm leading-7 text-[#f5efe3]/70 md:text-base">
              Explore public milestones
              and leave thoughtful
              feedback for people
              building their next
              chapter.
            </p>
