...
export const getMilestoneById = async (req, res) => {
    try {
        ...
        if (!milestone) {
            res.status(404).json({
                message: "Milestone not found or you do not have permission",
            });
            return;
        }
...
export const updateMilestoneVisibility = async (req, res) => {
    try {
        const userId = req.user?.userId;
...
export const getPublicMilestoneById = async (req, res) => {
    try {
        const milestone = await Milestone.findOne({
            _id: req.params.id,
            isPublic: true,
        ...
        if (!milestone) {
            res.status(404).json({
                message: "Public milestone not found or this milestone is private",
            });
            return;
        }
        res.status(200).json(milestone);
        ...
        console.error("Error getting public milestone:", error);
        res.status(500).json({
